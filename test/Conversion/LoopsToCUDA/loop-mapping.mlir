...
func @simple_tiling(%arg0: memref<64x64x64xf64>, %arg1: memref<64x64x64xf64>) {
  ...
  // CHECK: loop.parallel ({{%.*}}, [[ARG1_1:%.*]], [[ARG3:%.*]]) = ([[C0]], [[C0]], [[C0]]) to ([[C64]], [[C64]], [[C64]]) step ([[C64]], [[C2]], [[C1]]) {
  // CHECK-NEXT: loop.parallel ([[ARG0:%.*]], [[ARG1_2:%.*]], {{%.*}}) = ([[C0]], [[C0]], [[C0]]) to ([[C64]], [[C2]], [[C1]]) step ([[C1]], [[C1]], [[C1]]) {
...
func @predicate_execution(%arg0: memref<65x65x65xf64>, %arg1: memref<65x65x65xf64>) {
  ...
  // CHECK: loop.parallel ([[ARG0_1:%.*]], [[ARG1_1:%.*]], [[ARG3:%.*]]) = ([[C0]], [[C0]], [[C0]]) to ([[C128]], [[C66]], [[C65]]) step ([[C64]], [[C2]], [[C1]]) {
  // CHECK-NEXT: loop.parallel ([[ARG0_2:%.*]], [[ARG1_2:%.*]], {{%.*}}) = ([[C0]], [[C0]], [[C0]]) to ([[C64]], [[C2]], [[C1]]) step ([[C1]], [[C1]], [[C1]]) {
  loop.parallel (%arg2, %arg3, %arg4) = (%c0, %c0, %c0) to (%c65, %c65, %c65) step (%c1, %c1, %c1) {
...
func @predicate_execution_with_step(%arg0: memref<65x65x65xf64>, %arg1: memref<65x65x65xf64>) {
  ...
  %c3 = constant 3 : index
  %c65 = constant 65 : index
  // CHECK: loop.parallel ([[ARG0_1:%.*]], [[ARG1_1:%.*]], [[ARG3:%.*]]) = ([[C0]], [[C0]], [[C0]]) to ([[C128]], [[C66]], [[C65]]) step ([[C64]], [[C6]], [[C1]]) {
  // CHECK-NEXT: loop.parallel ([[ARG0_2:%.*]], [[ARG1_2:%.*]], {{%.*}}) = ([[C0]], [[C0]], [[C0]]) to ([[C64]], [[C6]], [[C1]]) step ([[C1]], [[C3]], [[C1]]) {
  loop.parallel (%arg2, %arg3, %arg4) = (%c0, %c0, %c0) to (%c65, %c65, %c65) step (%c1, %c3, %c1) {
    // CHECK-NEXT: [[IDX0:%.*]] = affine.apply #map{{[0-9]+}}([[ARG0_1]], [[ARG0_2]]) 
    // CHECK-NEXT: [[IDX1:%.*]] = affine.apply #map{{[0-9]+}}([[ARG1_1]], [[ARG1_2]]) 
    // CHECK-NEXT: [[CMP0:%.*]] = cmpi "slt", [[IDX0]], [[C65]] : index
    // CHECK-NEXT: [[CMP1:%.*]] = cmpi "slt", [[IDX1]], [[C65]] : index
