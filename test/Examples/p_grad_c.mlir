...
func @p_grad_c(
  ...
  %rdxc = stencil.load %rdxc_fd : (!stencil.field<ijk,f64>) -> !stencil.temp<ijk,f64>
  %rdyc = stencil.load %rdyc_fd : (!stencil.field<ijk,f64>) -> !stencil.temp<ijk,f64>
  %delpc = stencil.load %delpc_fd : (!stencil.field<ijk,f64>) -> !stencil.temp<ijk,f64>
  %gz  = stencil.load %gz_fd : (!stencil.field<ijk,f64>) -> !stencil.temp<ijk,f64>
  %pkc = stencil.load %pkc_fd : (!stencil.field<ijk,f64>) -> !stencil.temp<ijk,f64>
  // wk
  %wk = stencil.apply %arg1 = %delpc : !stencil.temp<ijk,f64> {
      %delpc_center = stencil.access %arg1[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
  ...
  !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, f64 {

      %gz_im1kp1 = stencil.access %arg3[-1, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %gz_center = stencil.access %arg3[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      %gz_im1kp1mcenter = subf %gz_im1kp1, %gz_center : f64

      %pkc_kp1 = stencil.access %arg4[0, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_im1 = stencil.access %arg4[-1, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_kp1mim1 = subf %pkc_kp1, %pkc_im1 : f64

      %prod_0 = mulf %gz_im1kp1mcenter, %pkc_kp1mim1 : f64

      %gz_im1 = stencil.access %arg3[-1, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      %gz_kp1 = stencil.access %arg3[0, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %gz_im1mkp1 = subf %gz_im1, %gz_kp1 : f64

      %pkc_im1kp1 = stencil.access %arg4[-1, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_center = stencil.access %arg4[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      ...
      %wk_im1 = stencil.access %arg2[-1, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      %wk_center = stencil.access %arg2[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64

      %wk_im1pcenter = addf %wk_im1, %wk_center : f64

      %rdxc_center = stencil.access %arg1[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
  ...
  !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, !stencil.temp<ijk,f64>, f64 {
      ...
      %gz_jm1kp1 = stencil.access %arg3[0, -1, 1] : (!stencil.temp<ijk,f64>) -> f64
      %gz_center = stencil.access %arg3[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      %gz_jm1kp1mcenter = subf %gz_jm1kp1, %gz_center : f64

      %pkc_kp1 = stencil.access %arg4[0, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_jm1 = stencil.access %arg4[0, -1, 0] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_kp1mjm1 = subf %pkc_kp1, %pkc_jm1 : f64

      %prod_0 = mulf %gz_jm1kp1mcenter, %pkc_kp1mjm1 : f64

      %gz_jm1 = stencil.access %arg3[0, -1, 0] : (!stencil.temp<ijk,f64>) -> f64
      %gz_kp1 = stencil.access %arg3[0, 0, 1] : (!stencil.temp<ijk,f64>) -> f64
      %gz_jm1mkp1 = subf %gz_jm1, %gz_kp1 : f64

      %pkc_jm1kp1 = stencil.access %arg4[0, -1, 1] : (!stencil.temp<ijk,f64>) -> f64
      %pkc_center = stencil.access %arg4[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
      ...
      %wk_jm1 = stencil.access %arg2[0, -1, 0] : (!stencil.temp<ijk,f64>) -> f64
      %wk_center = stencil.access %arg2[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64

      %wk_jm1pcenter = addf %wk_jm1, %wk_center : f64

      %rdyc_center = stencil.access %arg1[0, 0, 0] : (!stencil.temp<ijk,f64>) -> f64
