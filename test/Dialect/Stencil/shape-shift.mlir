...
func @lowerdim(%arg0: !stencil.field<ij,f64>, %arg1: !stencil.field<ijk,f64>) attributes {stencil.program} {
  ...
  stencil.assert %arg1 ([-3, -3, 0]:[67, 67, 60]) : !stencil.field<ijk,f64>
  %0 = stencil.load %arg0 ([0, 0, 0]:[64, 64, 60]) : (!stencil.field<ij,f64>) -> !stencil.view<ij,f64>
  %1 = stencil.apply %arg2 = %0 : !stencil.view<ij,f64> {
    %2 = stencil.access %arg2[0, 0, 0] : (!stencil.view<ij,f64>) -> f64
    stencil.return %2 : f64
  } to ([0, 0, 0]:[64, 64, 60]) : !stencil.view<ijk,f64>
  stencil.store %1 to %arg1 ([0, 0, 0]:[64, 64, 60]) : !stencil.view<ijk,f64> to !stencil.field<ijk,f64>
  return
}
...
//  CHECK-NEXT: stencil.assert %{{.*}} ([0, 0, 0]:[70, 70, 60]) : !stencil.field<ijk,f64>
//  CHECK-NEXT: %{{.*}} = stencil.load %{{.*}} ([3, 3, -9223372036854775808]:[67, 67, -9223372036854775808]) : (!stencil.field<ij,f64>) -> !stencil.view<ij,f64>

//       CHECK: %{{.*}} = stencil.access %{{.*}}[0, 0, -9223372036854775808] : (!stencil.view<ij,f64>) -> f64
