...
/*
// Make sure that only y direction has unroll parameter != 1
if (unrollAttr[0].cast<IntegerAttr>().getValue() != 1 || unrollAttr[2].cast<IntegerAttr>().getValue() != 1) {
  parser.emitError(parser.getCurrentLocation(),
                   "currently only unrolling in y direction is supported");
}
 */
...
struct UnrollingRewrite : public OpRewritePattern<stencil::ApplyOp> {
  UnrollingRewrite(MLIRContext *context)
...
void StencilUnrollingPass::runOnFunction() {
  ...
  patterns.insert<UnrollingRewrite, RerouteRewrite>(&getContext());
  applyPatternsGreedily(funcOp, patterns);
}

} // namespace

std::unique_ptr<OpPassBase<FuncOp>> mlir::stencil::createStencilUnrollingPass() {
